...
def fill_call(task_dict, arr_pair_for_call, arr_start_time, arr_end_time):
    ...
    for key, value in task_dict.items():
        if (value.user_assigned, value.id) in arr_pair_for_call:
            for start_time, end_time in product(arr_start_time, arr_end_time):
                call = Call(id, value.user_assigned, value.id, start_time, end_time, value.client_id)
                col_call.insert_one(call.__dict__())
                calls_dict[id] = call
                id += 1
    return calls_dict